...
TICKET_TYPE = ["Economy", "Premium Economy", "Business", "First Class"]
PASSENGER_TYPE = ["Adult", "Child", "Infant"]
WEAK_DAYS_LIST = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']


def util_round_zero(num):
    return round(0 if num is None else num * 100, 1)


def util_round_no_comma(num):
    # number =  '{0:,.4f}'.format(number)
    return round(0 if num is None else num, 4)


def util_round(num):
    return '{0:,.2f}'.format(round(0 if num is None else num, 2))


def util_round_with_comma(num):
    # number =  '{0:,.4f}'.format(number)
    # return   round(0 if num is None else num, 4)
    num = round(0 if num is None else num, 4)
    return '{0:,.2f}'.format(num) if num >= 0 else '({0:,.2f})'.format(abs(num))
...
def util_round_two(num):
    return round(0 if num is None else num, 2)


def util_round_money(num):
    num = round(0 if num is None else num, 4)
    return '$ {0:,.2f}'.format(num) if num >= 0 else '$ ({0:,.2f})'.format(abs(num))


def util_round_persentage(num):
    num = round(0 if num is None else num * 100, 3)
    return '{0} %'.format(num) if num >= 0 else '({0}) %'.format(abs(num))


def generate_color():
    return "#{0}".format(''.join([random.choice('0123456789ABCDEF') for _ in range(6)]))


def clone_model(original, **kwargs):
    """Clone an arbitrary sqlalchemy original object without its primary key values."""
    # Ensure the original’s data is loaded before copying.
    if original:
        table = original.__table__
        non_pk_columns = [k for k in table.columns.keys() if k not in table.primary_key]
        data = {c: getattr(original, c) for c in non_pk_columns}
        data.update(kwargs)
        return data
    return kwargs
...
def form_log(form):
    for fieldName, errorMessages in form.errors.items():
        for err in errorMessages:
            print("fieldName: ", fieldName, " Err: ", err)
...
def represents_int(s):
    try:
        int(s)
        return len(s) == 9 and s[0:2] in ['61', '69', '69']
    except ValueError:
        return False


def check_phone_number(to_emp_id, customer_tel):
    """
    :param customer_tel:
    :param to_emp_id:
    :return:
    """
    print("EMP tel util", to_emp_id)
    from web.blueprints.hrm.employee.models import Employee
    emp_mobile = Employee.query.filter(Employee.emp_id == to_emp_id).first()
    to_emp_mobile = represents_int(emp_mobile.mobile.strip()[1:])
    to_customer_phone = represents_int(customer_tel.strip())
    if to_emp_mobile is not True:
        flash('Please correct employee mobile number:' + emp_mobile.mobile + '.', 'danger')
        return True
    if to_customer_phone is not True:
        flash('Please correct Customer phone Number. ' + customer_tel + '.', 'danger')
        return True
    return False
...
countries = [
    ...
    {'timezones': ['Africa/Brazzaville'], 'code': 'CG', 'continent': 'Africa', 'name': 'Republic of the Congo',
     'capital': 'Brazzaville'},
    {'timezones': ['Africa/Abidjan'], 'code': 'CI', 'continent': 'Africa', 'name': "Cte d'Ivoire",
    ...
    {'timezones': ['Indian/Mauritius'], 'code': 'MU', 'continent': 'Africa', 'name': 'Mauritius',
     'capital': 'Port Louis'},
    {'timezones': ['Indian/Maldives'], 'code': 'MV', 'continent': 'Asia', 'name': 'Maldives', 'capital': 'Mal'},
    ...
    {'timezones': ['Africa/Sao_Tome'], 'code': 'ST', 'continent': 'Africa',
     'name': 'So Tom and Prncipe', 'capital': 'So Tom'},
    {'timezones': ['Asia/Damascus'], 'code': 'SY', 'continent': 'Asia', 'name': 'Syria', 'capital': 'Damascus'},
    {'timezones': ['Africa/Lome'], 'code': 'TG', 'continent': 'Africa', 'name': 'Togo', 'capital': 'Lom'},
...
calendar_year = [
    "2020",  # - 31 days
    "2021",  # - 28 days in a common year and 29 days in leap years
    "2022",  # - 31 days
    "2023",  # - 30 days
    "2024",  # - 31 days
    "2025",  # - 30 days
